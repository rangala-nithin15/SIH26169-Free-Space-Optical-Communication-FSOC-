...
import { Icon, Readout, fmt, fmtSigned } from './ui';
...
function FieldMap({ s }: { s: Snapshot }) {
  ...
  return (
    <svg className="mini" viewBox={`0 0 ${W} ${H}`}>
      <rect x={X(-hu)} y={Y(hv)} width={2 * hu * sc} height={2 * hv * sc} fill="rgba(143,220,255,0.03)" stroke="rgba(255,181,71,0.35)" strokeDasharray="2 3" />
      <line x1={X(0)} y1={Y(-hv)} x2={X(0)} y2={Y(hv)} stroke="rgba(170,214,255,0.08)" />
      <line x1={X(-hu)} y1={Y(0)} x2={X(hu)} y2={Y(0)} stroke="rgba(170,214,255,0.08)" />
      <rect x={X(bu) - fw / 2} y={Y(bv) - fh / 2} width={fw} height={fh} fill="rgba(143,220,255,0.1)" stroke={stateColor(s.state)} strokeWidth="1" />
      {s.gimbal.goalU !== null && s.gimbal.goalV !== null && <circle cx={X(s.gimbal.goalU)} cy={Y(s.gimbal.goalV)} r="2" fill="none" stroke="var(--amber)" />}
      <circle cx={X(s.target.u)} cy={Y(s.target.v)} r="2.6" fill="#e9f8ff" />
      <circle cx={X(s.target.u)} cy={Y(s.target.v)} r="5" fill="none" stroke="rgba(233,248,255,0.35)" />
    </svg>
  );
}
...
function GimbalDial({ s }: { s: Snapshot }) {
  ...
  return (
    <svg className="mini" viewBox="0 0 118 88">
      <circle cx={cx} cy={cy} r={R} fill="none" stroke="rgba(170,214,255,0.2)" />
      {Array.from({ length: 12 }).map((_, i) => {
        const a = (i * Math.PI) / 6;
        return <line key={i} x1={cx + (R - 4) * Math.sin(a)} y1={cy - (R - 4) * Math.cos(a)} x2={cx + R * Math.sin(a)} y2={cy - R * Math.cos(a)} stroke="rgba(170,214,255,0.3)" />;
      })}
      ...
      <path d="M86 78 A28 28 0 0 1 114 78" transform="rotate(-90 86 78)" fill="none" stroke="rgba(170,214,255,0.2)" />
      <line x1="86" y1="78" x2="114" y2="78" stroke="rgba(170,214,255,0.25)" />
...
function AqsGauge({ s }: { s: Snapshot }) {
  ...
  return (
    <svg className="mini" viewBox="0 0 118 88">
      <path d={arc(a0, a1)} stroke="rgba(170,214,255,0.14)" strokeWidth="5" fill="none" strokeLinecap="round" />
