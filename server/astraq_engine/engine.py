...
class SimulationEngine:
    ...
    def _control(self, ts: float, dt: float) -> tuple[float, float]:
        ...
        if st == "REACQUIRING" and self.reacq is not None and self.reacq_center is not None:
    ...
    def step(self) -> dict:
            ...
            if self.sup.state == "SEARCHING":
                ...
                if not self.kf.initialized:
                    ...
                    if not accepted and self.sup.state in ("LOST", "REACQUIRING"):
                        self.kf.init(maz, mel, cap["t"], max(r_deg, 0.01) * 2)
                        accepted = True
            else:
                accepted = True
            if accepted:
        ...
        if false_det:
            cerr = None
        self.metrics.update(self.t, self.sup.state, point_err, cerr if accepted else None, det.confidence if det else 0.0, result.proc_ms, false_det)
        tl = self.timeline
        s = self.sup.state
    ...
    def _snapshot(self, result, cap, accepted, meas_err, point_vec, point_err, ang_err, pred_px, decoy) -> dict:
        ...
        snap = {
            ...
            "disturbance": {"dPanDeg": self.dstate.d_pan, "dTiltDeg": self.dstate.d_tilt, "transmission": self.dstate.atm.transmission, "scint": self.dstate.scint, "dropout": self.dstate.dropout},
