...
class PixelKalman:
    ...
    def __init__(self, q: float = 400.0, r: float = 1.5):
    ...
    def step(self, dt: float, meas: tuple[float, float] | None, gate: float = 25.0) -> bool:
        if not self.ok:
            if meas is None:
                return False
            self.x.init(meas[0], 25, 400)
            self.y.init(meas[1], 25, 400)
            self.ok = True
            return True
        ...
        if nis > gate and math.hypot(nx, ny) > 40:
            return False
...
def analyze_frames(frames, fps: float, params: VideoParams, truth: dict[int, tuple[float, float]] | None = None) -> dict:
    ...
    for i, img in enumerate(frames):
        ...
        ctx = DetectionContext(pred, kf.ok and state != "SEARCHING", 60.0, params.spot_size_px, params.threshold_sigma, params.min_confidence, rng)
        frame = SensorFrame(W, H, img, None, None, False, params.spot_size_px, 0.0, None)
...
def synthesize_video(path: str, seconds: float = 6.0, fps: int = 30, size=(640, 480), seed: int = 3, noise: float = 8.0) -> str:
    """Write a test MP4 (noise + moving 10 px beacon on a Lissajous path) and return the
    ground-truth CSV text (frame,x,y). Used by tests and scripts/make_test_video.py."""
    if not HAVE_CV2:
    ...
    for i in range(n):
        ...
        x0, y0 = int(round(x - 5)), int(round(y - 5))
        img[max(0, y0) : y0 + 10, max(0, x0) : x0 + 10] += 220
        img += rng.normal(0, noise, (H, W))
        vw.write(np.clip(img, 0, 255).astype(np.uint8))
        # Pixel-centre convention: a block covering columns x0..x0+9 has centroid x0+5.
        lines.append(f"{i},{x0 + 5:.2f},{y0 + 5:.2f}")
    vw.release()
    return "\n".join(lines) + "\n"
